...
### 常见问题解答（FAQ）
- **Q:** 如何撤销错误的标注？  
  **A:** 使用快捷键 `Ctrl+Z` 撤回上一步。
- **Q:** 有哪些快捷键？  
  **A:** 上一张：`A`；下一张：`D`；切换标注模式：默认 `SAM`，`W`：矩形框，`Shift+W`：四点标注；  
  - 编辑开关：`E`（选中框后才能拖拽修改）  
  - 保存标签：`S`（弹出标签输入）  
  - 快速保存：`Alt+S`（使用列表第一个标签，不弹窗）  
  - 全局保存：`Ctrl+S`（写回 XML，包含已拖拽修改的框）  
  - 删除当前框：`Q`  
  - 撤销：`Ctrl+Z`  
  - 取消未确认框：`ESC`
- **Q:** 如何切换标注模式？  
  **A:** 默认 `SAM`；`W`：矩形框；`Shift+W`：四点标注。
- **Q:** 如何快速保存？  
  **A:** 使用 `Alt+S`。
- **Q:** 下次打开能否接着上次的图片继续标？  
  **A:** 可以。程序会把上次停留的图像文件名写入 `GUI/last_paths.json`；**下次启动自动打开上次的图片目录**时会直接跳到该图；若手动「打开目录」且选的是**同一文件夹**，也会恢复到最后一张正在标的图。
